...
public class StrictBankAccount implements BankAccount {
    ...
    public void depositFromATM(final int usrID, final double amount) {
        if (totalTransactionCount < maximumAllowedATMTransactions) {
        	...
        	throw new TransactionsOverQuotaException("ERRORE NUM.TRANSAZIONI", this.totalTransactionCount);
        }
    }
    ...
    private boolean checkUser(final int id) {
        if (this.usrID == id) {
        	...
        	throw new WrongAccountHolderException("ERRORE USER ID",id);
        }
    }
    ...
    private boolean isWithdrawAllowed(final double amount) {
        if (balance > amount) {
        	...
        	throw new NotEnoughFoundsException("ERRORE BALANCE", this.balance);
        }
    }
